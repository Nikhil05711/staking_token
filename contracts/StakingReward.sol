    ...
    RewardsDistributionRecipient
{
    ...
    //         _totalReward = (
    //             userStakedAmount[msg.sender].mul(totalRewardInPercentage).div(100)
    //         );
    ...
    //         );
    //     } else {
    //         rewardGenerateTime = claimableReward[msg.sender].div(_rewardPerSecond);
    //         claimableReward[msg.sender] = _rewardPerSecond.mul(
    //             rewardGenerateTime
    //         );
    //       }
    ...
    function claim(uint256 amount) public {
        if (_totalReward > 0) {
            ...
            require(coolingPeriod < block.timestamp,"Can Claim Capital After Cooling period");
            ClaimedTime[msg.sender] = block.timestamp;
            Rijent.transferPrice(address(this), msg.sender, _capitalPerWeek);
        } else {
            uint256 lastClaimedTime = block.timestamp.sub(ClaimedTime[msg.sender]);
            require(
                lastClaimedTime == ClaimedTime[msg.sender].add(604800),
